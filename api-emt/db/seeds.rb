...
10.times do
  ...
  project.rules.create(
    rule_string: "url?utm_source=<<#{dimensions.first.id}>>&&utm_medium=<<#{dimensions.last.id}>>"
  )
  # seed un-applied rule
  project.rules.create(
    rule_string: "url?utm_source=<<#{dimensions.last.id}>>&&utm_medium=<<#{dimensions.first.id}>>"
  )
end
